...
export const getMe = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [{ data: profile }, { data: roles }] = await Promise.all([
      supabase.from("profiles").select("id,name,email,school_id,status").eq("id", userId).maybeSingle(),
...
export const getDashboard = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [schools, students, companies, internships, evaluations] = await Promise.all([
      ...
      supabase.from("internships").select("id,status,student_id,start_date,period,competency,school_id"),
    ...
    const placedStudents = new Set(
      list.filter((i) => ["SUBMITTED", "APPROVED", "ACTIVE", "COMPLETED"].includes(i.status))
        .map((i) => i.student_id),
    );
    ...
    for (const i of list) competencyMap.set(i.competency, (competencyMap.get(i.competency) ?? 0) + 1);
    ...
    return {
      ...
      unplacedStudents:
        (students.data ?? []).filter((s) => s.status === "ACTIVE" && !placedStudents.has(s.id)).length,
      ...
      trendChart: [...trendMap.entries()].sort((a, b) => a[0].localeCompare(b[0]))
        .map(([month, value]) => ({ name: month, value })),
      competencyChart: [...competencyMap.entries()].sort((a, b) => b[1] - a[1])
...
export const saveSchool = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await context.supabase.from("audit_logs").insert({
      user_id: context.userId, action: id ? "UPDATE" : "CREATE", entity: "school",
      entity_id: id ?? null, detail: values.name,
    });
    return { ok: true };
  });
...
export const saveStudent = createServerFn({ method: "POST" })
  ...
  .inputValidator((input: unknown) =>
    z.object({
      ...
      birth_date: z.string().trim().min(10).max(10).optional().nullable(),
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await query;
    if (error) throw new Error("Data siswa gagal disimpan. Periksa nomor induk yang mungkin sudah dipakai.");
    await supabase.from("audit_logs").insert({
      user_id: userId, action: id ? "UPDATE" : "CREATE", entity: "student", entity_id: id ?? null, detail: values.name,
    });
    return { ok: true };
  });
...
export const saveCompany = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await context.supabase.from("audit_logs").insert({
      user_id: context.userId, action: id ? "UPDATE" : "CREATE", entity: "company",
      entity_id: id ?? null, detail: values.name,
    });
    return { ok: true };
  });
...
export const saveQuota = createServerFn({ method: "POST" })
  ...
  .inputValidator((input: unknown) =>
    z.object({
      ...
      competency: z.string().trim().min(2).max(80),
      period: z.string().trim().regex(/^\d{4}-S[12]$/, "Format periode: 2026-S1"),
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await query;
    if (error) throw new Error("Kuota gagal disimpan. Kombinasi perusahaan, kompetensi, dan periode harus unik.");
    return { ok: true };
  });
...
export const listInternships = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data, error } = await context.supabase
      .from("internships")
      .select("*, students(name, student_number), schools(name), companies(name), evaluations(final_score)")
...
export const getInternship = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [reports, attendance, evaluation, quota] = await Promise.all([
      supabase.from("internship_reports").select("*").eq("internship_id", data.id).order("report_date", { ascending: false }),
      supabase.from("attendance").select("*").eq("internship_id", data.id).order("date", { ascending: false }),
      supabase.from("evaluations").select("*").eq("internship_id", data.id).maybeSingle(),
      supabase.from("company_quotas").select("*").eq("company_id", internship.company_id)
        .eq("competency", internship.competency).eq("period", internship.period).maybeSingle(),
    ]);
    return {
      internship,
...
export const submitInternship = createServerFn({ method: "POST" })
  ...
  .inputValidator((input: unknown) =>
    z.object({
      ...
      company_id: z.string().uuid(),
      competency: z.string().trim().min(2).max(80),
      period: z.string().trim().regex(/^\d{4}-S[12]$/),
      start_date: z.string().min(10).max(10),
      end_date: z.string().min(10).max(10),
    }).parse(input),
  )
  .handler(async ({ data, context }) => {
    if (data.end_date <= data.start_date) throw new Error("Tanggal selesai harus setelah tanggal mulai.");
    ...
    if (error) {
      ...
      if (message.includes("QUOTA_FULL")) throw new Error("Kuota perusahaan untuk kompetensi ini sudah penuh.");
      if (message.includes("QUOTA_NOT_FOUND")) throw new Error("Perusahaan belum membuka kuota untuk kompetensi dan periode ini.");
      if (message.includes("STUDENT_ALREADY_PLACED")) throw new Error("Siswa ini sudah memiliki pengajuan atau penempatan aktif.");
...
export const decideInternship = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (error) {
      ...
      if (message.includes("INVALID_STATUS")) throw new Error("Pengajuan ini sudah diproses sebelumnya.");
      if (message.includes("FORBIDDEN")) throw new Error("Anda tidak memiliki akses ke tindakan ini.");
...
export const updateInternshipStatus = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) =>
    z.object({ id: z.string().uuid(), status: z.enum(["ACTIVE", "COMPLETED", "CANCELLED"]) }).parse(input),
  )
  .handler(async ({ data, context }) => {
    const { data: current } = await context.supabase
      .from("internships").select("status").eq("id", data.id).maybeSingle();
    ...
    const { error } = await context.supabase.from("internships").update({ status: data.status }).eq("id", data.id);
    if (error) throw new Error("Status gagal diperbarui. Silakan coba lagi.");
    await context.supabase.from("audit_logs").insert({
      user_id: context.userId, action: "UPDATE", entity: "internship", entity_id: data.id, detail: data.status,
    });
    return { ok: true };
  });
...
export const saveReport = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await context.supabase.from("audit_logs").insert({
      user_id: context.userId, action: "CREATE", entity: "internship_report", entity_id: data.internship_id,
...
export const saveEvaluation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await context.supabase.from("audit_logs").insert({
      user_id: context.userId, action: "EVALUATE", entity: "internship", entity_id: data.internship_id,
      detail: "Penilaian diperbarui",
    });
    const { data: saved } = await context.supabase
      .from("evaluations").select("*").eq("internship_id", data.internship_id).maybeSingle();
...
export const getForecast = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const [demand, quotas, companies] = await Promise.all([
      context.supabase.from("competency_demand").select("competency,location,period,requested_quota"),
    ...
    const series = [...groups.values()].map((g) => forecastSeries(g.competency, g.location, toPoints(g.points)));

    const byCompetency = new Map<string, { history: Map<string, number>; locations: Set<string> }>();
    ...
    const competencyForecast = [...byCompetency.entries()]
      .map(([competency, entry]) => forecastSeries(competency, "Semua Lokasi", toPoints(entry.history)))
    ...
    const topCompetencies = competencyForecast.filter((c) => c.sufficient).slice(0, 4).map((c) => c.competency);
    ...
    return {
      series,
      competencyForecast,
      locations: [...locationTotals.entries()].sort((a, b) => b[1] - a[1]).map(([name, total]) => ({ name, total })),
...
export const createGuruUser = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (error || !created.user) throw new Error("Pengguna gagal dibuat. Email mungkin sudah terdaftar.");
    await supabaseAdmin.from("profiles")
      .upsert({ id: created.user.id, name: data.name, email: data.email, school_id: data.school_id });
    await supabaseAdmin.from("user_roles").delete().eq("user_id", created.user.id);
    await supabaseAdmin.from("user_roles").insert({ user_id: created.user.id, role: data.role });
    await context.supabase.from("audit_logs").insert({
      user_id: context.userId, action: "CREATE", entity: "user", entity_id: created.user.id, detail: data.email,
    });
    return { ok: true };
  });
...
export const updateUser = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await context.supabase.from("profiles")
      .update({ name: data.name, school_id: data.school_id, status: data.status }).eq("id", data.id);
    ...
    await context.supabase.from("audit_logs").insert({
      user_id: context.userId, action: "UPDATE", entity: "user", entity_id: data.id, detail: data.status,
    });
    return { ok: true };
  });
...
export const globalSearch = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [students, schools, companies] = await Promise.all([
      context.supabase.from("students").select("id,name,student_number,competency").ilike("name", term).limit(5),
