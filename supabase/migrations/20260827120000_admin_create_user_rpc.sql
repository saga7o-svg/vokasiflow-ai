-- Migration: 20260827120000_admin_create_user_rpc.sql
-- Description: Create dedicated RPC for Super Admin to create active users in auth.users and public.profiles
...
BEGIN
  ...
  IF target_uid IS NULL THEN
    ...
    INSERT INTO auth.users (
      ...
      jsonb_build_object('name', target_name, 'role', assigned_role::text, 'school_id', resolved_school_id),
      'authenticated',
      'authenticated',
      now(),
      now()
    ...
    INSERT INTO auth.identities (
      ...
      target_uid,
      target_uid,
      jsonb_build_object('sub', target_uid::text, 'email', clean_email),
      'email',
      clean_email,
      now(),
      now(),
  ...
  ELSE
    ...
    UPDATE auth.users
       SET encrypted_password = extensions.crypt(target_password, extensions.gen_salt('bf')),
           ...
           raw_user_meta_data = jsonb_build_object('name', target_name, 'role', assigned_role::text, 'school_id', resolved_school_id),
           updated_at = now()
     WHERE id = target_uid;
